...
export const TypewriterEffect = ({
  ...
  return (
    <div
      className={cn(
        "text-center font-bold text-white sm:text-xl md:text-3xl lg:text-5xl",
        className,
      )}
    >
      {renderWords()}
      ...
      <div>
        {wordsArray.map((word, idx) => {
          return (
            <div
              key={`word-${idx}`}
              className="inline-block text-xs sm:text-sm md:text-base lg:text-lg xl:text-xl 2xl:text-2xl"
            >
              {word.text.map((char, index) => (
                <span
                  key={`char-${index}`}
                  className={cn(
                    `text-white`,
                    word.className,
                    // Adicionando tamanhos de texto responsivos
